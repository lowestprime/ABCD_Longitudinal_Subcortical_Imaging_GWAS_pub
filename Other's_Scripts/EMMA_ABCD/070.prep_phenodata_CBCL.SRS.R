#### Format phenotype and covariate files ####
rm(list=ls())

# libraries
suppressPackageStartupMessages(suppressWarnings({
    library(tidyverse)
    library(FRGEpistasis)
    library(patchwork)
}))


#### directories and files ####
date <- "120422"
batchinfodir <- "/u/project/gandalm/shared/GenomicDatasets/ABCD_Release_4/genomics_sample03"
badiddir <- "/u/project/gandalm/shared/GenomicDatasets/ABCD_Release_4/genomics_sample03/genotype_QCed"
qcdir <- "/u/project/gandalm/emmamk/ASD_GWAS/ABCDr4/02.QCplink"
ancestry_id_dir <- "/u/project/gandalm/shared/GenomicDatasets-processed/ABCD_Release_4/genotype/TOPMed_imputed"
# can use the bigsnpr files
pcdir <- "/u/project/gandalm/shared/GenomicDatasets-processed/ABCD_Release_4/genotype/ancestry_PCs"
phenodir <- "/u/project/gandalm/shared/GenomicDatasets/ABCD_Release_4/phenotypes"
shared_phenodir <- "/u/project/gandalm/shared/GenomicDatasets-processed/ABCD_Release_4/phenotype"
plotdir <- paste(shared_phenodir, "plots", sep = "/")
if (!dir.exists(plotdir)) {dir.create(plotdir)}
scores <- list(cbcl = "cbcl_scr_syn_social_t", srs = "ssrs_p_ss_sum")


#### Read in phenotype files ####
setwd(phenodir)
# check file names
list.files(pattern = "cbcl")
list.files(pattern = "srs")

# read in the files
files <- list.files(pattern = "abcd_cbcls01.txt|abcd_mhp02.txt")
names(files) <- c("cbcl", "srs")

pheno <- list()
for (file in files) {
    if (file == "abcd_cbcls01.txt") {
        score = scores[["cbcl"]]
    } else {
        score = scores[["srs"]]
    }
    print(file)
    print(score)
    dt <- read.delim(file, header = TRUE, na.strings = c("", "NA"))
    dt <- dt[-c(1),]
    dt <- dplyr::select(dt, subjectkey, eventname, all_of(score), interview_age, sex)

    age_col <- "interview_age"        
    dt[,c(score, age_col)] <- lapply(dt[,c(score, age_col)], as.numeric)
    pheno[[file]] <- dt
}

# check the data
lapply(pheno, head, n = 3)
lapply(pheno, nrow)

names(pheno) <- names(files)

#. quick check on the pheno data ----
for (i in seq_along(pheno)) {
    dt <- pheno[[i]]
    score <- scores[[i]]
    
    # https://stackoverflow.com/questions/63403373/r-how-to-summarize-and-group-by-variables-as-column-names
    print(
        dt %>% group_by(eventname, sex) %>% 
            summarise(n = n(),
                      across(interview_age, mean, na.rm = TRUE), 
                      across(all_of(score), mean, na.rm = TRUE), .groups = 'drop')
        )
}
# CBCL has data for all timepoints whereas SRS has only for 1_year_follow_up_y_arm_1
# Age are the same across measures in ABCD study as long as using the same timepoint/eventname.
# For ASD_GWAS project, we will use `1_year_follow_up_y_arm_1` for CBCL and SRS.

#. filter to 1_year_follow_up_y_arm_1 ----
for (i in seq_along(pheno)) {
    print(names(pheno[i]))
    dt <- pheno[[i]]
    print(paste("nrow(dt_original):", nrow(dt)))
    dt <- dt %>% filter(eventname == "1_year_follow_up_y_arm_1")
    print(paste("nrow(dt_1yr_followup):", nrow(dt)))
    pheno[[i]] <- dt
    cat("\n")
}


#### Tidy up phenodata ####
for (i in seq_along(pheno)){
    
    dt <- pheno[[i]]
    name <- names(pheno[i])
    samplesize_original <- nrow(dt)
    
    cat("########## ", toupper(name), " ##########\n")
    
    ### Remove samples have NAs in the target score
    cat("### Removing NAs in the target score column ###\n")
    score <- ifelse(str_detect(name, "cbcl"), scores[[1]],
                   ifelse(str_detect(name, "srs"), scores[[2]], NA))
    nrow_before <- nrow(dt)
    print(paste0("samplesize before: ", nrow_before))
    
    print("NAs in the target score:")
    print(table(is.na(dt[,score])))
    cat("\n")

    # remove NAs
    keep <- !is.na(dt[,score])
    dt <- dt[keep,]
    
    nrow_after <- nrow(dt)
    diff <- nrow_before - nrow_after
    
    print(paste0("samplesize after: ", nrow_after))
    print(paste0(diff, " samples removed (NAs in the score column)"))
    cat("\n")
    
    
    ### Remove samples have NAs in the age column
    cat("### Removing NAs in the age column ###\n")
    age_col <- "interview_age"
    nrow_before <- nrow(dt)
    print(paste0("samplesize before: ", nrow_before))
    
    print("NAs in the target score:")
    print(table(is.na(dt[,age_col])))
    cat("\n")

    # remove NAs
    keep <- !is.na(dt[,age_col])
    dt <- dt[keep,]
    
    nrow_after <- nrow(dt)
    diff <- nrow_before - nrow_after
    
    print(paste0("samplesize after: ", nrow_after))
    print(paste0(diff, " samples removed (NAs in the age column)"))
    cat("\n")    
      
    
    ### All: get necessary columns
    cat("### Subsetting to necessary columns ###\n")
    print(paste0("ncol() before: ", ncol(dt)))
    dt <- dt %>% dplyr::select(subjectkey, all_of(age_col), sex, all_of(score))
    print(paste0("ncol() after: ", ncol(dt)))
    cat("\n")
    
    
    ### print final dimention of the data
    cat("### Sample size ###\n")
    samplesize_processed <- nrow(dt)
    diff <- samplesize_original - samplesize_processed
    print(paste0("original: ", samplesize_original))
    print(paste0("processed: ", nrow(dt)))
    print(paste0("unique samples: ", length(dt$subjectkey)))
    print(paste0(diff, " samples removed through the process"))
    cat("\n\n")
    
    pheno[[i]] <- dt
}

# nrow() after tiding up the data
lapply(pheno, nrow)


#### Correct inconsistent IDs ####
setwd(badiddir)
dt_id_issue <- read.delim("id_inconsistent_INVF3FYXH1G_INVPWLFYWTX_replacement.txt", header = FALSE, na.strings = c("", "NA"))
names(dt_id_issue) <- c("oldFID", "oldIID", "newFID", "newIID")
rownames(dt_id_issue) <- dt_id_issue$newIID
head(dt_id_issue)

#. fix the inconsistent IDs (if any) ----
for (i in seq_along(pheno)) {
    print(names(pheno[i]))
    print(dt[grep("`NDAR|INVF3FYXH1G|INVPWLFYWTX", dt$subjectkey),])
    cat("\n\n") 
}
# [1] "cbcl"
#            subjectkey interview_age sex ssrs_p_ss_sum
# 2789 NDAR_INVPWLFYWTX           121   M            12


# [1] "srs"
#            subjectkey interview_age sex ssrs_p_ss_sum
# 2789 NDAR_INVPWLFYWTX           121   M            12

# Notes:
# - `INVF3FYXH1G` does not have CBCL nor SRS data  
# - The ID `INVPWLFYWTX` looks alright.

#. sanity check for the IDs ----
for (i in seq_along(pheno)){
    dt <- pheno[[i]]
    # fix the IDs
    print("before replacing the IDs")
    print(dt[grep("INVF3FYXH1G|INVPWLFYWTX", dt$subjectkey),])

    if ("`NDAR_INVF3FYXH1G" %in% dt$subjectkey) {
        print(dt[grep("INVF3FYXH1G", dt$subjectkey),])
        dt[grep("INVF3FYXH1G", dt$subjectkey),] <- str_replace(dt[grep("INVF3FYXH1G", dt$subjectkey),], "`NDAR", "NDAR")
    } else {
        print("no problem for `NDAR_INVF3FYXH1G")
    }
    
    
    if ("NDARINVPWLFYWTX" %in% dt$subjectkey) {
        print(dt[grep("NDARINVPWLFYWTX", dt$subjectkey),])
        dt[grep("INVF3FYXH1G", dt$subjectkey),] <- str_replace(dt[grep("INVF3FYXH1G", dt$subjectkey),], "`NDAR", "NDAR")
    } else {
        print("no problem for NDARINVPWLFYWTX")
    }
    print("after replacing/checking the IDs")
    print(dt[grep("INVF3FYXH1G|INVPWLFYWTX", dt$subjectkey),])
        
    # store the FID corrected data to the list
    pheno[[i]] <- dt
    cat("\n\n")
}

dt <- pheno[[1]]
dt[grep("INVF3FYXH1G|INVPWLFYWTX", dt$subjectkey),]
# no output == no problem


#### Modify IDs with ABCD ID format ####
setwd(batchinfodir)
Batch <- read.delim("ABCD_release3.0_.batch_info.txt",  header = FALSE, na.strings = c("", "NA"))
Batch <- Batch[-c(1),]; nrow(Batch)  # 11099
head(Batch)

# list inconsistent IDs
Batch[grep("INVF3FYXH1G|INVPWLFYWTX", Batch$V2),2]
# '`NDAR_INVF3FYXH1G''NDARINVPWLFYWTX'

Batch[grep("INVF3FYXH1G", Batch$V2),]

# fix the IDs
Batch[grep("INVF3FYXH1G", Batch$V2),] <- str_replace(Batch[grep("INVF3FYXH1G", Batch$V2),], "`NDAR", "NDAR")
Batch[grep("INVPWLFYWTX", Batch$V2),] <- str_replace(Batch[grep("INVPWLFYWTX", Batch$V2),], "NDARINVPWLFYWTX", "NDAR_INVPWLFYWTX")
Batch[grep("INVF3FYXH1G|INVPWLFYWTX", Batch$V2),2]
# 'NDAR_INVF3FYXH1G''NDAR_INVPWLFYWTX'

Batch$IID <- paste(Batch$V1, Batch$V2, sep = "_")
Batch<- rename(Batch, subjectkey = V2)
Batch$IID2 <- Batch$IID
head(Batch)

# sanity check
Batch[grep("INVF3FYXH1G|INVPWLFYWTX", Batch$subjectkey),]

IID <- dplyr::select(Batch, IID, IID2, subjectkey)


#### Merge with phenodata ####
for (i in seq_along(pheno)) {
    dt <- pheno[[i]]
    print(paste("original nrow():", nrow(dt)))

    dt <- merge(dt, IID, by = 'subjectkey')
    print(paste("merged nrow():", nrow(dt)))
    
    dt <- dt %>% dplyr::select(IID, IID2, 2:4)
    print(head(dt, 3))
    
    pheno[[i]] <- dt
    cat("\n\n")
}


#### Split by ancestry and gender ####
setwd(ancestry_id_dir)
list.files(pattern = "IDs")

# IDs for each ancestry group
afr <- read.table("ABCDr4_AFR.2174_no.sexmismatch_IDs.txt")
amr <- read.table("ABCDr4_AMR.1798_no.sexmismatch_IDs.txt")
eas <- read.table("ABCDr4_EAS.278_no.sexmismatch_IDs.txt")
eur <- read.table("ABCDr4_EUR.6679_no.sexmismatch_IDs.txt")
sas <- read.table("ABCDr4_SAS.88_no.sexmismatch_IDs.txt")
ancestry_samples_id_list <- list(AFR = afr$V2, AMR = amr$V2, EAS = eas$V2, EUR = eur$V2, SAS = sas$V2)

# before splitting into ancestry groups
lapply(ancestry_samples_id_list, head, n = 3)
lapply(pheno, dim)

# split by ancestry
pheno_by_ancestry <- list()
for (i in seq_along(pheno)) {
    pheno_data <- pheno[[i]]
    pheno_name <- tolower(names(pheno[i]))
    
    for (j in seq_along(ancestry_samples_id_list)) {
        ids <- ancestry_samples_id_list[[j]]
        ancestry_name <- tolower(names(ancestry_samples_id_list[j]))
        name <- paste(pheno_name, ancestry_name, sep = "_")
        pheno_by_ancestry[[name]] <- pheno_data %>% filter(IID %in% ids)       
    }
}
# after splitting into ancestry groups
lapply(pheno_by_ancestry, dim)

# split phenodata to ancestry groups
seq_along(pheno_by_ancestry)
pheno_by_ancestry_and_gender <- list()
gender <- c("M", "F")
for (i in seq_along(pheno_by_ancestry)) {
    pheno_data <- pheno_by_ancestry[[i]]
    pheno_name <- tolower(names(pheno_by_ancestry[i]))
    
    for (j in seq_along(gender)) {
        name <- paste(pheno_name, tolower(gender[j]), sep = "_")
        pheno_by_ancestry_and_gender[[name]] <- pheno_data %>% filter(sex == gender[j])
    }
}
# after splitting into ancestry groups
lapply(pheno_by_ancestry_and_gender, dim)
length(pheno_by_ancestry_and_gender)

#. combine phenodata lists ----
data <- c(pheno_by_ancestry, pheno_by_ancestry_and_gender)
length(data)
names(pheno)
head(pheno_by_ancestry[[1]])

#. save tables for each phenotype containing ancestry info ----
pheno_by_ancestry_before_rint <- list()
n <- length(pheno_by_ancestry) # 25 (5 pheno * 5 ancestry groups)
for (i in seq_along(pheno)){
    pheno_name <- names(pheno[i])
    keyword <- paste0(pheno_name, "_")
    pheno_dt_list <- data[1:n][str_detect(names(data[1:n]), keyword)]
    pheno_by_ancestry_before_rint[[pheno_name]] <- do.call("rbind", pheno_dt_list)
}
lapply(pheno_by_ancestry_before_rint, head, n = 3)
lapply(pheno_by_ancestry_before_rint, nrow)

# check if all samples are unique
lapply(pheno_by_ancestry_before_rint, function(dt){
    return(length(unique(dt$IID)) == nrow(dt))
})

# save tables
setwd(shared_phenodir)
tables <- pheno_by_ancestry_before_rint
for (i in seq_along(tables)) {
    pheno_name <- names(tables[i])
    
    dt <- tables[[i]]
    n <- nrow(dt)
    file_name <- paste0(pheno_name, "_1yr_followup_before.rint_within_ancestry_group_noNAs_", n, "_table_", date, ".csv")
    print(file_name)
    write.csv(dt, file_name, row.names = TRUE)
}


#### Rank transform phenotypes ####
for (i in seq_along(data)){
    dt <- data[[i]]
    name <- names(data[i])    
    score <- ifelse(str_detect(name, "cbcl"), scores[[1]],
                   ifelse(str_detect(name, "srs"), scores[[2]], NA))
    score_rint <- paste(score, "rint", sep = "_")
    age_col <- "interview_age"
    age_col_years <- "interview_age_years"
    age_col_years_nodecimal <- "interview_age_years_nodecimal"
    n <- nrow(dt)
    
    dt[,c(score, age_col)] <- lapply(dt[,c(score, age_col)], as.numeric)
    dt[,age_col_years] <- round(dt[,age_col]/12, 2)
    dt[,age_col_years_nodecimal] <- round(dt[,age_col]/12, 0)
    dt[,score_rint] <- rankTransPheno(dt[,score], 0.5)
    data[[i]] <- dt
}

#. histogram ----
summary(data[[1]]$interview_age_years_nodecimal)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   10.00   10.00   11.00   10.91   11.00   12.00 

summary(factor(data[[1]]$interview_age_years_nodecimal))
# 10: 651  11: 825  12: 478

# histograms
setwd(plotdir)
for (i in seq_along(data)){
    dt <- data[[i]]
    name <- names(data[i])    
    score <- ifelse(str_detect(name, "cbcl"), scores[[1]],
                   ifelse(str_detect(name, "srs"), scores[[2]], NA))
    score_rint <- paste(score, "rint", sep = "_")
    age_col <- "interview_age_years"
    n <- nrow(dt)

    png(paste("hist", name, score, n, "png", sep = "."), width = 800, height = 800)
    par(mfrow = c(2,2))
    # score
    hist(dt[,score], main = paste0("Histogram: " , toupper(name), " (n=", n, ")", "\n", score), xlab = score, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
    hist(dt[,score_rint], main = paste0("Histogram: " , toupper(name), " (n=", n, ")", "\n", score_rint), xlab = score_rint, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
    # age in months
    hist(dt[,age_col], main = paste0("Histogram: " , toupper(name), " (n=", n, ")", "\n", age_col), xlab = age_col, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
    dev.off()    

    cat("\n\n")
}

#. histograms for each ancestry by gender ----
data <- lapply(data, function(dt){
    dt$sex <- factor(dt$sex, levels = c("M", "F"))
    return(dt)
})

setwd(plotdir)
for (i in seq_along(pheno_by_ancestry)){
    dt <- data[[i]]
    name <- names(data[i])    
    score <- ifelse(str_detect(name, "cbcl"), scores[[1]],
                   ifelse(str_detect(name, "srs"), scores[[2]], NA))
    
    score_rint <- paste(score, "rint", sep = "_")
    n <- nrow(dt)
    
    p1 <- ggplot(dt, aes(x = dt[,score], fill = sex, color = sex)) +
        geom_histogram(position="identity", alpha=0.5, binwidth = 5) +
        theme_bw() +
        labs(title = paste("Histogram:", toupper(name)),
             subtitle = paste0(score, " (n=", n, ")"), 
             x        = "Scores",
             y        = "Freqency") +
        theme(text = element_text(size = 16), legend.position='none', panel.grid = element_blank()) +
        scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
        scale_color_manual(values = c("#00BFC4", "#F8766D")) +
        guides(fill = guide_legend("Gender"), color = guide_legend("Gender"))

    p2 <- ggplot(dt, aes(x = dt[,score_rint], fill = sex, color = sex)) +
        geom_histogram(position="identity", alpha=0.5, binwidth = 0.5) +
        theme_bw() +
        labs(title = paste("Histogram:", toupper(name)),
             subtitle = paste0(score_rint, " (n= ", n, ")"), 
             x        = "Scores",
             y        = "") +
        guides(fill = guide_legend("Gender"), color = guide_legend("Gender")) +
        theme(text = element_text(size = 16), legend.position='none', panel.grid = element_blank()) +
        scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
        scale_color_manual(values = c("#00BFC4", "#F8766D")) +
        guides(fill = guide_legend("Gender"), color = guide_legend("Gender"))

#     suppressWarnings(print(p1 + p2 + plot_layout(guides = "collect") & 
#                 theme(legend.position = "right", legend.text = element_text(size = 14))))

    png(paste("hist", name, score, "by_gender", n, "png", sep = "."), width = 800, height = 400)    
    suppressWarnings(print(p1 + p2 + plot_layout(guides = "collect") & 
            theme(legend.position = "right", legend.text = element_text(size = 14))))
    dev.off()
}


#. histograms for each ancestry by age ----
data <- lapply(data, function(dt){
    age_median <- median(dt$interview_age_years)
    dt$age_median <- ifelse(dt$interview_age_years > age_median, "top50%", "bottom50%")
    dt$age_median <- factor(dt$age_median, levels = c("top50%", "bottom50%"))
    return(dt)
})

setwd(plotdir)
for (i in seq_along(pheno_by_ancestry)){
    dt <- data[[i]]
    name <- names(data[i])    
    score <- ifelse(str_detect(name, "cbcl"), scores[[1]],
                   ifelse(str_detect(name, "srs"), scores[[2]], NA))
    
    score_rint <- paste(score, "rint", sep = "_")
    n <- nrow(dt)
    
    p1 <- ggplot(dt, aes(x = dt[,score], fill = age_median, color = age_median)) +
        geom_histogram(position="identity", alpha=0.5, binwidth = 5) +
        theme_bw() +
        labs(title = paste("Histogram:", toupper(name)),
             subtitle = paste0(score, " (n=", n, ")"), 
             x        = "Scores",
             y        = "Freqency") +
        theme(text = element_text(size = 16), legend.position='none', panel.grid = element_blank()) +
        scale_fill_manual(values = c("orange", "skyblue1")) +
        scale_color_manual(values = c("orange", "skyblue1")) +
        guides(fill = guide_legend("Age by Median"), color = guide_legend("Age by Median"))

    p2 <- ggplot(dt, aes(x = dt[,score_rint], fill = age_median, color = age_median)) +
        geom_histogram(position="identity", alpha=0.5, binwidth = 0.5) +
        theme_bw() +
        labs(title = paste("Histogram:", toupper(name)),
             subtitle = paste0(score_rint, " (n= ", n, ")"), 
             x        = "Scores",
             y        = "") +
        guides(fill = guide_legend("Age by Median"), color = guide_legend("Age by Median")) +
        theme(text = element_text(size = 16), legend.position='none', panel.grid = element_blank()) +
        scale_fill_manual(values = c("orange", "skyblue1")) +
        scale_color_manual(values = c("orange", "skyblue1")) +
        guides(fill = guide_legend("Age by Median"), color = guide_legend("Age by Median"))

#     suppressWarnings(print(p1 + p2 + plot_layout(guides = "collect") & 
#                 theme(legend.position = "right", legend.text = element_text(size = 14))))

    png(paste("hist", name, score, "by_age_median", n, "png", sep = "."), width = 800, height = 400)    
    suppressWarnings(print(p1 + p2 + plot_layout(guides = "collect") & 
            theme(legend.position = "right", legend.text = element_text(size = 14))))
    dev.off()
}


#### Combine phenodata ####
#. by ancestry ----
pheno_by_ancestry_rint <- list()
n <- length(pheno_by_ancestry) # 35 (7 pheno * 5 ancestry groups)
for (i in seq_along(pheno)){
    pheno_name <- names(pheno[i])
    keyword <- paste0(pheno_name, "_")
    pheno_dt_list <- data[1:n][str_detect(names(data[1:n]), keyword)]
    pheno_by_ancestry_rint[[pheno_name]] <- do.call("rbind", pheno_dt_list)
}
lapply(pheno_by_ancestry_rint, nrow)
lapply(pheno_by_ancestry_rint, head, n = 3)

#. check if all samples are unique ----
lapply(pheno_by_ancestry_rint, function(dt){
    return(length(unique(dt$IID)) == nrow(dt))
})

# age summary ----
lapply(pheno_by_ancestry_rint, function(dt){
    return(summary(dt$interview_age_years))
})
# $cbcl
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    9.67   10.33   10.92   10.93   11.50   12.42 

# $srs
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    9.67   10.33   10.92   10.93   11.50   12.42 

#. save tables with RINT values ----
setwd(shared_phenodir)
for (i in seq_along(pheno_by_ancestry_rint)) {
    pheno_name <- names(pheno_by_ancestry_rint[i])
    dt <- pheno_by_ancestry_rint[[i]]
    n <- nrow(dt)
    file_name <- paste0(pheno_name, "_1yr_followup_after.rint_within_ancestry_group_noNAs_age.as.is_", n, "_table_", date, ".csv")
    print(file_name)
    write.csv(dt, file_name, row.names = TRUE)
}

#. save phenotype files ----
setwd(shared_phenodir)
scores <- list(cbcl = "cbcl_scr_syn_social_t_rint", srs = "ssrs_p_ss_sum_rint")
for (i in seq_along(pheno_by_ancestry_rint)) {
    pheno_name <- names(pheno_by_ancestry_rint[i])
    dt <- pheno_by_ancestry_rint[[i]]
        score <- ifelse(str_detect(pheno_name, "cbcl"), scores[[1]],
                   ifelse(str_detect(pheno_name, "srs"), scores[[2]], NA))
    n <- nrow(dt)
    file_name <- paste0("Phen_AllSubj_", pheno_name, "_1yr_followup_", score, "_within_ancestry_group_noNAs_", n, ".txt")
    print(file_name)
    
    dt <- dt %>% dplyr::select(IID, IID2, all_of(score))
    print(head(dt, 3))
    write.table(dt, file_name, col.names = FALSE, row.names = FALSE, sep = "\t", quote = FALSE)
    cat("\n")
}


#### Create qcovar files ####
# read in ancestry PCs
setwd(pcdir)
files <- list.files(pattern = "eigenvec")
files

#### IDs for each ancestry group ####
pcs <- list()
for (i in seq_along(files)) {
    file <- files[i]
    ancestry <- tolower(str_extract(file, "AFR|AMR|EAS|EUR|SAS"))
    pcs[[ancestry]] <- read.table(file, na.strings = c("", "NA"))
}
lapply(pcs, head, n = 3)

pcs <- do.call("rbind", pcs)
dim(pcs)

names(pcs) <- c("fid", "iid", paste0(rep("PC", 10), 1:10))
head(pcs, 3)
tail(pcs, 3)

#. save quantitative covariate files ----
setwd(shared_phenodir)
for (i in seq_along(pheno_by_ancestry_rint)) {
    par(mfrow = c(1,2))
    pheno_name <- names(pheno_by_ancestry_rint[i])
    print(pheno_name)

    dt <- pheno_by_ancestry_rint[[i]]
    dt$ancestry <- str_extract(rownames(dt), "afr|amr|eas|eur|sas")
    dt_pcs <- merge(dt, pcs, by.x = "IID", by.y = "iid")
    print(paste("nrow(dt_pcs):", nrow(dt_pcs)))
    
    # PCs1-10
    dt_pcs10 <- dt_pcs %>% dplyr::select(IID, IID2, interview_age_years_nodecimal, paste0(rep("PC", 10), 1:10))
    print(head(dt_pcs10, 3))
    n <- nrow(dt_pcs10)
    print(n)

    file_name <- paste0("qcovar_AllSubj_", pheno_name, "_1yr_followup_age.as.is_PCs1-10_within_ancestry_group_noNAs_", n, ".txt")
    print(file_name)
    write.table(dt_pcs10, file_name, col.names = FALSE, row.names = FALSE, sep = "\t", quote = FALSE)
    cat("\n")
}


#### Create discrete covariate files ####
head(Batch, 3)
names(Batch)[4] <- "batch"

nrow(Batch)
table(Batch$batch)
table(is.na(Batch$batch))

#. merge with gender info ----
setwd(phenodir)
dt_tracking <- read.delim("abcd_lt01.txt", na.strings = c("", "NA"))
dt_tracking <- dt_tracking[-1,]
head(dt_tracking, 3)

# check inconsistent IDs
dt_tracking[grep("INVF3FYXH1G|INVPWLFYWTX", dt_tracking$subjectkey),4]
# seems no issues

# get baseline data
dt_tracking <- dt_tracking %>% filter(eventname == "baseline_year_1_arm_1") %>%
    dplyr::select(subjectkey, interview_age, sex)

covar <- merge(Batch, dt_tracking, by = "subjectkey")
nrow(covar)
head(covar, 3)

# format covariate file
covar <- covar %>% dplyr::select(IID, IID2, batch, sex)
head(covar, 3)

summary(factor(covar$sex))
# F: 5214  M: 5849
summary(factor(covar$batch))
# BATCH_1_Saliva: 618  BATCH_1_WB: 11  BATCH_2_Saliva: 4358  BATCH_2_WB: 194  BATCH_3_Saliva: 4017  BATCH_4_Saliva: 1865


#. save discrete covariate file ----
setwd(shared_phenodir)

# batch, gender
batch_gender <- covar
n <- nrow(batch_gender)
filename <- paste0("covar_AllSubj_batch_gender_noNAs_baseline_", n, ".txt")
write.table(batch_gender, filename, col.names = FALSE, row.names = FALSE, sep = "\t", quote = FALSE)

